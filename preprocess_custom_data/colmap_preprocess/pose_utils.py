...
def load_colmap_data(realdir):
    ...
    for k in imdata:
        ...
        intr = np.eye(4)
        intr[0, 0] = cam.params[0]
        intr[1, 1] = cam.params[1]
        intr[0, 2] = cam.params[2]
        intr[1, 2] = cam.params[3]
        intr_mats.append(intr)
    ...
    intr_mats = np.stack(intr_mats, axis=-1)

    return poses, intr_mats, pts3d, perm


def save_poses(basedir, poses, intr_mats, pts3d, perm):
    pts_arr = []
    vis_arr = []
    for k in pts3d:
    ...
    intr_mats = np.moveaxis(intr_mats, -1, 0)
    intr_mats = intr_mats[perm]
    np.save(os.path.join(basedir, 'intr_mats.npy'), intr_mats)
...
def gen_poses(basedir, match_type, factors=None):
    ...
    poses, intr_mats, pts3d, perm = load_colmap_data(basedir)

    save_poses(basedir, poses, intr_mats, pts3d, perm)
